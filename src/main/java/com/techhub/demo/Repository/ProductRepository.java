...
public class ProductRepository {
	...
	{
		int value = jdbcTemplate.update("insert into Product values (?,?,?, ?, ?, (select pcid from ProdCategory where pcname =?))",new PreparedStatementSetter()
				{
					@Override
	...
	{
		int value = jdbcTemplate.update("update Product set ProdName=?,Description=?,Description=?,Stock=?,pcid=? where ProdId=?",new PreparedStatementSetter() {
	...
	{
		list=jdbcTemplate.query("select *from Product where ProdName=?",new Object[] {Name},new RowMapper<ProductModel>() {
