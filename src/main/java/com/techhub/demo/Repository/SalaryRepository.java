...
public class SalaryRepository {
	...
	public boolean isAddNewSalaryUser(SalaryModel sal) {
		int value = jdbcTemplate.update(
				"insert into Salary values (?,(select Rid from RegistrationMaster where name=? and (role = 'Employee' OR role = 'Cashier')), ?)",
	...
	public boolean isUpdateEmpSalary(SalaryModel sal) {
		int value = jdbcTemplate.update("update Salary set salary=?,(select Rid from RegistrationMaster where name=?)",
				new PreparedStatementSetter() {
	...
	public List<SalaryModel> getAllEmpInfo() {
		list = jdbcTemplate.query(
				"SELECT r.Rid,r.Name, r.Email, r.Role,r.Address,r.PhoneNo ,s.Salary_Id,s.Salary,r.Password FROM Salary s INNER JOIN RegistrationMaster r ON s.Rid = r.Rid;\r\n",
